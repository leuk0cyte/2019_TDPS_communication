...
INT1 = 6                            # 对应树莓派3b+的引脚为：40,38,37,35,36,33
INT2 = 5
INT3 = 1
INT4 = 7
PWMA = 0
...
RIGHT_MOTOR_PARAM = 1                 # 分别定义左右电机的运转系数，使得两电机保持相同的速度
LEFT_MOTOR_PARAM  = 0.7               # 小于1
##############################################################################
...
def motor_angle(degree):              # 原地转动特定的角度，正值为右负值为左
    if degree == -45:                    # 左45度
        ...
        pwmb.ChangeDutyCycle(50)
        time.sleep(1.6)
    elif degree == -2:                 #turn left for 2 degree
        pwma.ChangeDutyCycle(50)
        pwmb.ChangeDutyCycle(0)
        time.sleep(0.07)
    elif degree == 2:                  #turn right for 2 degree
        pwma.ChangeDutyCycle(0)
        pwmb.ChangeDutyCycle(50)
        time.sleep(0.04)
    elif degree == -5:  # 左5度
